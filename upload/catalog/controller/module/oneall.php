...
namespace Opencart\Catalog\Controller\Extension\OneAllSocialLogin\Module;
...
class Oneall extends \Opencart\System\Engine\Controller
{
    ...
    public function register()
    {
        ...
        if ($this->is_default_group_behaviour())
        {
            if (is_array($this->config->get('config_customer_group_display')))
            {
                ...
                {
                    if (
                        in_array($customer_group['customer_group_id'],
    ...
    private function validate()
    {
        ...
        {
            if (
                isset($this->request->post['customer_group_id']) && is_array($this->config->get('config_customer_group_display')) && in_array($this->request->post['customer_group_id'],
                    $this->config->get('config_customer_group_display'))
            )
            {
    ...
    public function callback()
    {
        ...
        if (isset($this->request->post) && !empty($this->request->post['connection_token']))
        {
                            ...
                            if (!empty($setting_auto_link))
                            {
                                ...
                                if (!empty($user_data['user_email']) && isset($user_data['user_email_is_verified']) && $user_data['user_email_is_verified'] === true)
                                {
                                    ...
                                    if (is_numeric($customer_id_tmp))
                                    {
                                        // Link the customer to this social network.
                                        if (
                                            $this->link_tokens_to_customer_id($customer_id_tmp,
            ...
            {
                $this->add_log("Could not process Social Login, the API subdomain is missing");
            }
        }
        // No OneAll POST data received
        else
        {
            $this->response->redirect($this->url->link('common/home', '', true));
        }
    ...
    protected function login_customer($customer_id)
    {
        ...
        $result = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer` WHERE customer_id = '" . intval($customer_id) . "'")->row;
    ...
    public function extract_social_network_profile($reply)
    {
        // Check API result.
        if (
            is_object($reply) && property_exists($reply,
