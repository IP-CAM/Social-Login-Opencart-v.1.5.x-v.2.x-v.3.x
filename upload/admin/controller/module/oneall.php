...
class OneAll extends \Opencart\System\Engine\Controller
{
    ...
    protected function index_settings($data)
    {
        ...
        {
            // Social Networks
            if (isset($this->request->post['module_oneall_social_networks']))
            {
                if (is_array($this->request->post['module_oneall_social_networks']))
                {
                    $oneall_socials = array();
                    
                    foreach ($this->request->post['module_oneall_social_networks'] as $key => $is_enabled)
                    {
                        if (!empty($is_enabled))
                        {
                            $oneall_socials[] = $key;
                    ...
                    $this->request->post['module_oneall_socials'] = implode(",", $oneall_socials);
                }
            }
            
            // OneAll API Subdomain
            if (isset($this->request->post['module_oneall_subdomain']))
            {
                // Remove Spaces
            ...
            if (isset($this->request->post['module_oneall_public']))
            {
                // Remove Spaces
                $this->request->post['module_oneall_public'] = trim($this->request->post['module_oneall_public']);
            }
            
            // OneAll API Private Key
            if (isset($this->request->post['module_oneall_private']))
            {
                // Remove Spaces
                $this->request->post['module_oneall_private'] = trim($this->request->post['module_oneall_private']);
            }
        ...
        if (isset($data['module_oneall_api_handler']) && $data['module_oneall_api_handler'] == 'fso')
        {
            $data['module_oneall_api_handler'] = 'fso';
        }
        else
        {
            $data['module_oneall_api_handler'] = 'crl';
        }
        
        // Communication Port
        if (isset($data['module_oneall_api_port']) && $data['module_oneall_api_port'] == '80')
        {
            $data['module_oneall_api_port'] = '80';
        }
        else
        {
            $data['module_oneall_api_port'] = '443';
        }
        
        // Subdomain
        if (!isset($data['module_oneall_subdomain']))
        {
            $data['module_oneall_subdomain'] = '';
        }
        
        // Public Key
        if (!isset($data['module_oneall_public']))
        {
            $data['module_oneall_public'] = '';
        }
        
        // Private Key
        if (!isset($data['module_oneall_private']))
        {
            $data['module_oneall_private'] = '';
        }
        
        // Account Creation
        if (!isset($data['module_oneall_auto_account']) || !in_array($data['module_oneall_auto_account'], array(0, 1)))
        {
            $data['module_oneall_auto_account'] = '1';
        }
        
        // Account Creation - Address
        if (!isset($data['module_oneall_ask_address']) || !in_array($data['module_oneall_ask_address'], array(0, 1)))
        {
            $data['module_oneall_auto_account'] = '0';
        }
        
        // Account Link
        if (!isset($data['module_oneall_auto_link']) || !in_array($data['module_oneall_auto_link'], array(0, 1)))
        {
            $data['module_oneall_auto_link'] = '1';
        }
        
        // Customer Groups
        $data['oa_customer_groups'] = $this->getCustomerGroups();
        if (isset($data['module_oneall_customer_group']))
        {
            $data['oa_customer_group_selected'] = $data['module_oneall_customer_group'];
        }
        else
        {
        ...
        // Library Language
        if (!isset($data['module_oneall_store_lang']) || !in_array($data['module_oneall_store_lang'], array(0, 1)))
        {
            $data['module_oneall_store_lang'] = 1;
        }
                ...
                array(0, 1)))
        {
            ...
            {
                $data['module_oneall_deferred_loading'] = 1;
            }
            // Otherwise disable deferred loading
            else
            {
                $data['module_oneall_deferred_loading'] = 0;
            }
        }
        ...
        // Social Login Status
        $data['module_oneall_status'] = !empty($data['module_oneall_status']) ? 1 : 0;
    ...
    public function verify_api_settings($lang)
    {
        ...
        // Parse arguments
        $oneall_subdomain = (!empty($get['module_oneall_subdomain']) ? trim($get['module_oneall_subdomain']) : '');
        $oneall_public = (!empty($get['module_oneall_public']) ? trim($get['module_oneall_public']) : '');
        $oneall_private = (!empty($get['module_oneall_private']) ? trim($get['module_oneall_private']) : '');
        $oneall_api_handler = (!empty($get['module_oneall_api_handler']) ? trim($get['module_oneall_api_handler']) : '');
        $oneall_api_port = (!empty($get['module_oneall_api_port']) ? trim($get['module_oneall_api_port']) : '');
    ...
    private function getEvents()
    {
        $events = array(
            array(
                'trigger' => 'catalog/view/common/column_left/before',
